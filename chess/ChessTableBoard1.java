...
public final class ChessTableBoard1 implements ChessBoard {
	 ...
	 * 5 - m - 					? seconds
	 * 
	 *  Wie man in den Messergebnissen sehen kann, bringt Multithreading
	 *  hier nur einen m‰ﬂigen Erfolg
	 */
	/**
	 * {@inheritDoc}
	 */
	public List<short[]> getActiveMoves () {
		...
		final Semaphore indebtedSemaphore;
		final List<short[]> mustCaptureKingMoves = Collections.synchronizedList(new ArrayList<short[]>(256));
		final List<short[]> normalMoves = Collections.synchronizedList(new ArrayList<short[]>(256));
		
		
		final boolean whiteActive = this.isWhiteActive();
		if (this.getKingPosition(whiteActive) < 0) return normalMoves;

		final int sectorWidth = this.pieces.length / PROCESSOR_COUNT;
		final int sectorThreshold = this.pieces.length % PROCESSOR_COUNT;
		indebtedSemaphore = new Semaphore(1 - PROCESSOR_COUNT);
		...
		for (int threadIndex = 0; threadIndex < PROCESSOR_COUNT; ++threadIndex) {
			...
			final Runnable runnable = new Runnable() {
				public void run () {
					try {
						...
						List<short[]> moves = new ArrayList<short[]>(256);
						for (int leftIndex = startIndex; leftIndex < stopIndex; ++leftIndex) {
							final ChessPiece piece = pieces[leftIndex];
							if (piece != null && piece.isWhite() == whiteActive)
								mustCaptureKing = collectMoves(moves, piece, mustCaptureKing); //lever out the method ???
						}
						if(mustCaptureKing)
						{
							for(short[] move : moves)
							{
								mustCaptureKingMoves.add(move);
							}
						}
						else
						{
			...
			//EXECUTOR_SERVICE.execute(new CollectMoves(startIndex, stopIndex, whiteActive));
		}

		indebtedSemaphore.acquireUninterruptibly();
		
		if(mustCaptureKingMoves.isEmpty())
		{
			return normalMoves;
		}
		/* The old stuff
	...
	/*
	class CollectMoves implements Runnable,AutoCloseable
	{
		...
		public CollectMoves(int startIndex, int stopIndex, boolean whiteActive){
			super();
